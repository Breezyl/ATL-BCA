...
def load_embedding(setting):
    ...
    f = open('./data/embed.txt','r',encoding="utf-8")
    f.readline()
    while True:
        content=f.readline()
        if content=='':
...
def process_train_data(setting):
    ...
    train_label=[]
    train_length=[]
    f=open('./data/adrNER.conll.train', 'r', encoding='UTF-8')
    train_word.append([])
    train_label.append([])
    train_max_len=0
    while True:
    ...
    np.save('./data/train_length.npy', train_length)
...
def process_test_data(setting):
    ...
    test_label=[]
    test_length=[]
    f=open('./data/adrNER.conll.test','r',encoding="UTF-8")
    test_word.append([])
    test_label.append([])
    test_max_len=0
    while True:
    ...
    np.save('./data/test_length.npy', test_length)
...
def preprocess(filename):
    ...
    np.save('./data/cws_word.npy', sentence)
    np.save('./data/cws_label.npy', label)
    np.save('./data/cws_length.npy', length)
...
filename = 'data/cws_training.utf8'
preprocess(filename)
vector = np.asarray(vector)
np.save('./data/vector.npy',vector)
