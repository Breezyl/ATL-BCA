...
def evaluate(pred_batch,label_batch,length_batch,word_batch):
    ...
    for i in range(len(pred_batch)):
        ...
        for j in range(length_batch[i]):
            gold.append(preprocess.id_tag[label_batch1[j]])
            pred.append(preprocess.id_tag[pred_batch[i][j]])
            one_unit=[preprocess.id2word[word_batch[i][j]], gold[j], pred[j]]
            result.append(" ".join(one_unit))
        result_batch.append(result)
    return result_batch

def compute_f1(results):
    output_file='./data/output_adr.txt'
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            with tf.variable_scope('ner_model'):
                m = model.TransferModel(setting, tf.cast(embedding, tf.float32), adv=True, is_train=False)
                m.multi_task()
